...
export default async function OrderSuccessPage({ params, searchParams }: { params: Promise<{ locale: Locale }>; searchParams: Promise<{ order?: string; access?: string; mode?: string }> }) {
  ...
  return (<div className="container-x section"><div className="mx-auto max-w-2xl"><h1 className="t-section font-[family-name:--font-display]">{t("title")}</h1><p className="t-body mt-5 text-[--color-text-muted]">{t("lead")}</p><OrderStatusView orderNumber={order ?? null} accessToken={access ?? null} mode={mode ?? null} /></div></div>);
}
